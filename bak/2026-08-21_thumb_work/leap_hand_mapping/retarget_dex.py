...
기본 경로가 아니다 (`--retargeter dex`)
-------------------------------------
기본은 retarget.py(직접 구현한 손끝 위치 IK)다. 이유는 아래 장점의 이면에 있다.
벡터만 맞추므로 손끝 위치를 만족하는 한 MCP 굽힘은 널스페이스에 남는다. 실측하면
사람이 주먹을 쥘 때(MCP 60.3도) 이쪽 해는 MCP 를 -0.3도로 **펴고** 굽힘을 PIP/DIP 에
몰아넣는다. 목표는 잘 맞지만 사람 눈에는 주먹이 아니라 갈고리다.
재현: scripts/phase1/p1_diag_pose_fidelity.py

엄지-검지 핀치는 여전히 이쪽이 훨씬 낫다(주먹에서 32.9mm 대 96.6mm). 집기가 중요한
국면에서 `--retargeter dex` 로 바꿔 쓰라고 남겨 둔다.

왜 벡터인가 (직접 만든 retarget.py 와의 차이)
--------------------------------------------
