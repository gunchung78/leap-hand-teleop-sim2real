...
- 출력은 공식 관절 범위(`LEAPsim_limits()`, `joint_map.LIMITS_MJ`)로 클립한 뒤에만 실기로 나간다.
...
- "엄지만 붙임"은 결합이 오히려 **나쁘게** 한다(134 -> 186, 한계표 정정 뒤 200). 사람 간격
  106mm x 1.89 = 200mm 를 목표로 주는데, 그 자리는 엄지를 뒤로 젖혀야 닿는 곳이다.
  "사람의 1.89배"라는 가정이 엄지에서는 안 맞는다(실측 엄지 배율 1.55).
  (한때 여기에 "LEAP 엄지는 손바닥을 가로질러 안 접힌다"고 적었다. **틀렸다** — 실물은
  접힌다. 당시 `th_mcp` 가 -27도에 붙었던 것은 시뮬 한계표가 좁아서였고, 아래 "시뮬 엄지
  한계표 정정" 절에서 고쳤다. 고친 뒤 이 자세의 엄지 잔차는 14.6 -> 2.0mm, `th_mcp` 는
  -35도로 자유롭게 간다 — 즉 한계가 아니라 **목표(200mm)가 틀린 것**이 본질이다.)
...
#### 엄지 — 모양은 관절각 매핑, 핀치는 IK (적용됨, `--thumb-mode map`)

위 결합은 핀치를 줄였지만 사용자가 실제로 보면 두 가지가 남았다: **손을 펴면 엄지가
꺾여 있고, 손가락을 편 채 엄지를 손바닥에 붙여도 안 붙는다.** 렌더로 확인하면 편 손에서
`th_mcp` 77도(결합 켬) 또는 `th_axl` -20 / `th_mcp` -19(결합 끔)였다. 손끝 **목표점**은
1~5mm 로 맞추고 있었는데 **자세**가 틀렸다. LEAP 엄지는 축 배치가 사람과 달라(`th_axl` 은
축이 손끝을 지나 손끝 위치로는 관측이 안 된다) 손끝 하나로 자세가 결정되지 않는다.

네 손가락은 IK 가 잘 되므로 **엄지만** 바꿨다. LEAP 공식 `mano_to_leap_mapping.py` 가 하는
대로 관절각을 직접 옮긴다 — 그쪽은 MANO 해부학 각(CMC 벌림/굽힘, MCP, IP)을 이득 1 로
`th_cmc/th_axl/th_mcp/th_ipl` 에 싣고 "반드시 튜닝하라"고 적어 두었다. 여기서는 MANO 대신
MediaPipe 점으로 같은 뜻의 각을 재고, **편 손 캘리브레이션 대비 변화량**에 이득을 곱한다
(`retarget.thumb_features` / `_thumb_map`):

```
th_cmc = 2.0 x (휴지 - 중간마디 평면각)     엄지 중간마디가 새끼 쪽으로 돌수록 들린다
th_axl = 0                                 굽힘 평면 = 손가락 쪽 (렌더로 고름)
th_mcp = 2.0 x (MCP 굽힘 - 휴지)
th_ipl = 1.0 x (IP 굽힘 - 휴지)
```

편 손이면 정확히 영점 = LEAP 의 편 손이다. **편 손이 편 손으로 보이는 것을 구조로
보장한다.** 핀치는 그대로 IK 다 — 사람 엄지-검지 간격 50mm 위면 매핑 100%, 30mm 밑이면
IK 100%(결합 목표 = 검지 끝), 사이는 관절 공간에서 보간. 네 손가락은 손대지 않는다.

이득은 이렇게 정했다: 편 손 -> 엄지 붙임에서 사람 중간마디가 54도 돌고 MCP 가 24도 더
굽는다. LEAP 이 "손바닥을 가로질러 접힌" 모양이 되려면 렌더상 `th_cmc` 105~120 /
`th_mcp` 50~60 이 필요하다(`th_cmc` 는 손가락 축 둘레 회전이라 120 에서 손바닥 위로
눕는다). 54x2=108, 24x2=48. IP 는 사람 변화량이 그대로 보기 좋아 1.0. 튜닝 값이고
`thumb_gain_*` 로 바꿀 수 있다.

![엄지 ik 대 map](docs/img/thumb_ik_vs_map.png)

왼쪽 두 열이 예전(`--thumb-mode ik`), 오른쪽 두 열이 지금(`map`). 녹화 4자세
(`thumb_capture.npz`), 위/옆 시점. 숫자:

```
               th_cmc th_axl th_mcp th_ipl   엄지-검지mm   지터
편 손    ik        4     18     76    -29       134     0.78
         map       1      0      0     -0       248     0.79    <- 영점
엄지붙임  ik       41      4    -27     77       187     1.33
         map     107      0     47     40       122     0.88    <- 손바닥 위로 눕는다
핀치     ik       83     -9     57     -6        52     0.97
         map      83     -9     57     -6        52     0.97    <- 같다 (IK 구간)
주먹     ik       19     12     36     57        63     4.11
         map      90      3     66     39        75     2.74    <- 손가락 위를 감싼다
```

`p1_2_test_retarget_roundtrip.py` 는 `thumb_mode="ik"` 로 돈다 — 그 시험은 손끝 잔차로
좌표계를 검증하는데 매핑 엄지는 손끝을 쫓지 않아 잔차가 뜻이 없다. 같은 이유로
`p1_3_teleop_mujoco.py` 의 "손끝잔차" 표시도 map 모드에서는 엄지를 뺀 세 손가락 값이다.

#### 시뮬 엄지 한계표 정정 — menagerie 범위가 공식과 어긋난다

실물 LEAP 엄지는 손바닥을 가로질러 접히는데 시뮬에서는 `th_mcp` 가 -27도에서 막혔다.
한계표를 출처별로 대조해 보니 **menagerie `right_hand.xml` 이 틀려 있었다.**

| 관절 | 모터 | menagerie MJCF | 공식: 출처 URDF = `LEAPsim_limits()` | 예전 클립(교집합)이 잃던 것 |
|---|---|---|---|---|
| th_cmc | 12 | [-20°, +120°] | [-20°, +120°] | 없음 |
| **th_axl** | 13 | [-20°, +120°] | [**-27°, +140°**] | 위 20° |
| **th_mcp** | 14 | [-27°, +140°] | [**-69°, +109°**] | **아래 42°** |
| th_ipl | 15 | [-77°, +108°] | [-77°, +108°] | 없음 |

URDF 의 joint 12/13 값이 MJCF 의 th_axl/th_mcp 에 한 칸씩 밀려 들어가 있다. 출처
URDF(dex-urdf)와 `LEAP_Hand_API` 의 `LEAPsim_limits()` 는 서로 일치하므로 공식 두 출처가
맞고 menagerie 만 다르다(CHANGELOG 에 "Fixed Left Leap Hand's Thumb CMC range" 라는 같은
부류의 수정이 있다 — 오른손 쪽은 못 잡은 것으로 보인다).

그래서 "MJCF ∩ 실기" 교집합이라는 개념을 버리고 **관절 범위의 기준을 공식 하나로 통일**했다:
`joint_map.LIMITS_MJ_LOWER/UPPER` = `LEAPsim_limits()` 를 MuJoCo 순서로 옮긴 것(16개 전부
출처 URDF 와 동일). 클립·IK·트윈·실기 명령이 전부 이 표를 쓴다. MJCF 모델 자체는 여전히
좁게 로드되므로 `jm.apply_model_limits(model)` 을 로드 직후 불러 `jnt_range` / `ctrlrange`
를 공식값에 맞춘다(텔레오퍼레이션 본체, 스윕, FK 검증, 진단 전부). 안 맞추면 디지털
트윈이 명령 -57도에 -27도로 답한다(실측). 공식 범위에서 MuJoCo 자기충돌은 0 이다
(`th_mcp` 단독 -69도, `th_cmc` 86~115도 조합 모두). 어긋난 관절 목록은 `joint_map.MJCF_MISMATCH`.

바뀐 것과 안 바뀐 것(`p1_diag_thumb_options.py --thumb-mode ik`, 같은 녹화):

```
                    핀치mm        엄지잔차mm     th_mcp
엄지만 붙임   186.1 -> 200.1   14.6 -> 2.0   -26.9(한계) -> -34.9
편 손 / 핀치 / 주먹            변화 없음 (한계에 닿지 않는 자세)
```

기본 경로(`--thumb-mode map`)는 양의 `th_mcp` 만 쓰므로 표가 안 바뀐다. FK 교차검증
(std 0.05mm)과 왕복 시험(손끝 잔차 0.33mm)은 넓힌 범위에서도 통과한다.

#### 정직한 현재 상태

| 자세 | `ours` (기본) | `dex` |
|---|---|---|
| 편 손 | 영점 (엄지 곧게 옆으로) | 엄지 OK, 손가락 PIP 0 |
| 주먹 모양 | 주먹 (MCP/PIP/DIP 33/64/117), 엄지가 감싼다 | 갈고리 (26/3/115) |
| 엄지 붙임 | 손바닥 위로 눕는다 | — |
| 핀치 간격 (사람 27) | 52mm (IK 구간 그대로) | 38mm |
| 손목 회전 시 떨림 | 크다 (센서 탓) — 지원 범위 밖으로 뒀다 | 작다 |

핀치 구간의 IK 는 손대지 않았다. 남은 건 그 52mm 다.

### 남은 것

- **핀치 구간(IK)의 52mm.** 엄지 모양은 매핑으로 해결했고 핀치는 예전 IK 그대로다.
  매핑 엄지를 IK 의 시드/사전항으로 쓰면 IK 가 엄지 모양을 지킨 채 검지로 갈 여지가 있다.
- 실사용 검증: 실제 손으로 돌려 보고 `--dip-weight`·평활 계수·`thumb_gain_*` 을 조정.
