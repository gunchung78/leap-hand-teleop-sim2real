...
- "엄지만 붙임"은 결합이 오히려 **나쁘게** 한다(134 -> 186). 사람 간격 106mm x 1.89 =
  200mm 를 목표로 주는데, 그 자리는 엄지를 뒤로 젖혀야 닿는 곳이라 `th_mcp` 가
  하한(-27도)에 붙는다. "사람의 1.89배"라는 가정이 엄지에서는 안 맞는다(실측 엄지 배율
  1.55). (한때 여기에 "LEAP 엄지는 손바닥을 가로질러 안 접힌다"고 적었다. **틀렸다** —
  실물은 접힌다. 시뮬 한계표가 좁을 뿐이다: menagerie `right_hand.xml` 의 `th_mcp` 하한
  -27도, 공식 URDF 는 -69도. 아래 "엄지 — 모양은 관절각" 절의 방식은 양의 `th_mcp` 만
  써서 이 한계와 무관하다.)
...
- 시뮬 엄지 한계표. menagerie `th_mcp` 하한 -27도는 공식 URDF(-69도)보다 좁다. ik 모드와
  디지털 트윈이 그 값에 막힌다. `joint_map` 교집합과 MJCF `jnt_range` 를 공식값으로 넓힐지
  실기 한계와 같이 볼 것.
