...
base (현재)        242.3   227.1   147.8   105.5      58.1   1.17   6.4
...
- **A2 가 답이다.** 엄지 목표를 검지 목표 기준으로 상대 배치하고(간격 = 사람 간격 x
  검지 배율 1.91), 사람 간격이 30mm 밑이면 목표를 0 으로 눌러 "붙여라"로 바꾼다.
...
- **엄지 A2 적용.** 위 표의 근거는 `p1_diag_thumb_options.py` 에 있고 아직 본 경로에는
  안 들어갔다. `compute_targets` 에서 엄지 목표 두 개를 옮기는 20줄 안쪽 변경이다.
