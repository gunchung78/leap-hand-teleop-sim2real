...
### ⚠ 사람 데이터 오염 — 아래 표들은 재녹화 후 갱신한다

위 "자세 충실도" 표와 아래 "엄지" 표의 **사람 열은 잘못됐다.** 녹화(`p1_diag_thumb.py
--save`)가 타이머 기반이라 "핀치 3초" 라고 띄우고 바로 세기 시작했고, 사용자가 자세를
잡는 **도중**이 그대로 들어갔다. `thumb_capture.npz` 를 프레임별로 보면:

```
검지-엄지 핀치  94프레임   간격(mm) 시계열   50 98 97 98 96 96 95 93 94 93 51 21 23 23 24 24 ...
엄지만 붙임     94프레임                    75 74 74 74 74 74 74 74 73 67 55 53 52 52 ...
```

핀치 녹화의 2/3 는 손을 펴고 있었고(95mm) 진짜 핀치는 마지막 1/3(23mm) 뿐이다. 그
평균이 "사람 핀치 57.2mm" 로 표에 올라갔다. 로봇 쪽 숫자도 같은 프레임을 평균했으니
핀치 행은 사실상 "손을 펴는 중" 이었다.

각 자세의 **안정된 마지막 30프레임만** 다시 재면 (엄지-검지 간격 mm):

```
              사람    결합 끔   결합 켬
편 손          73.7    242.1    142.1
엄지만 붙임     52.1    215.0    109.4
검지-엄지 핀치  23.8    157.7     20.0    <- 핀치가 실제로 닫힌다
주먹           32.0     84.3     65.1
```

즉 A2 결합은 표에 적힌 것(99mm)보다 훨씬 잘 된다 — 사람 23.8mm 에 로봇 20.0mm.
다만 이것도 오염된 데이터의 꼬리를 쓴 잠정치다. 결론을 박기 전에 **재녹화**한다:

```bash
python scripts/phase1/p1_diag_record_poses.py    # 자세마다 STABLE 확인 후 SPACE
```

새 녹화기는 타이머가 없고, 화면에 MediaPipe 가 읽는 간격/굽힘각을 실시간으로 띄우며,
녹화 중 움직인 프레임은 버린다. "편 손 회전" 자세(모양 고정, 손목만 회전)도 추가했다
— 거기서 관절각이 흔들리면 센서/손목 프레임 문제, 안 흔들리면 리타겟팅 문제다.

### 엄지 — 원인과 해법 (A2 적용됨)

`ours` 를 기본으로 되돌린 뒤 남은 문제는 엄지다. 원인을 세 축으로 갈라 쟀다
(`python scripts/phase1/p1_diag_thumb_options.py`, 재현 가능).

**IK 가 아니라 목표가 틀렸다.** 사용자가 엄지를 검지에 붙이는 동안:

```
사람 엄지-검지 간격    57.2 mm
로봇에 준 목표 간격   149.0 mm      <- 목표부터 벌어져 있다
로봇이 실제로 만든 간격 147.8 mm     엄지 잔차 3.6mm  <- IK 는 목표를 정확히 달성
```

손가락마다 **자기 뿌리를 원점으로 자기 배율로** 목표를 만들고(검지 1.91, 엄지 1.57)
손가락끼리 묶는 항이 없다. 잔차를 아무리 낮춰도 안 고쳐진다.

**th_axl 은 관측이 거의 안 된다.** 회전축이 엄지 손끝을 거의 지나서, 1 rad 당 손끝이
12~22mm 밖에 안 움직인다(`th_cmc` 는 135mm). 손끝만 보는 IK 에서 사실상 자유변수라
한계(-20°)에 붙어 버린다. `th_cmc` 도 가동범위 120° 중 32° 만 쓴다(dex 는 107°).

후보를 같은 자로 쟀다. 엄지-검지 간격(mm):

```
                   편 손   엄지붙임   핀치    주먹     MCP(주먹)   지터   ms
사람                73.2    61.8    57.2    29.5      60.3      -     -
base (결합 끔)     242.3   227.1   147.8   105.5      58.1   1.17   6.4
C   th_axl=0       240.5   227.1   148.1   105.5      58.1   1.17   6.4
A1  투영만          242.3   227.1    80.3    54.2      58.1   1.17   6.4
A2  투영+상대배치    141.3   124.2    99.0    54.2      58.1   1.13   6.5
A2C A2 + th_axl=0  137.9   120.4    98.3    54.8      58.1   2.26   6.5
hybrid 엄지만 dex   126.7   112.9    94.2    95.1      58.1   1.11  11.8
dex (참고)         136.9   128.5    92.9    31.6      -0.3   1.98   3.7
now (적용된 코드)   141.3   124.2    99.0    54.2      58.1   1.13   6.5
```

읽는 법:

- **A2 를 채택해 본 경로에 넣었다**(`retarget._couple_thumb`, 끄려면
  `--no-thumb-couple`). 엄지 목표를 검지 목표 기준으로 상대 배치하고(간격 = 사람
  간격 x 검지 배율 1.91), 사람 간격이 30mm 밑이면 목표를 0 으로 눌러 "붙여라"로 바꾼다.
  두 상수는 DexPilot 의 `project_dist`/`escape_dist` 를 그대로 쓴다.
  편 손 242 -> 141mm 는 오차가 아니라 **비례**다. 로봇이 사람보다 1.91배 크다.
- **MCP 열이 전혀 안 변한다.** 엄지 관절과 네 손가락 손끝은 자코비안이 분리돼 있어
  (손가락 손끝은 엄지 관절에 안 달렸다) 엄지 목표를 바꿔도 자세 충실도는 그대로다.
  잃는 것 없이 얻는 변경이다.
- **`th_axl` 고정은 단독으로 무의미하다.** A2 를 넣으면 목표가 제대로 서면서
  `th_axl` 이 스스로 +30° 까지 쓰기 시작한다. A2C 는 지터만 2배(1.13 -> 2.26).
- **hybrid 는 기각.** 주먹에서 95.1mm 로 오히려 나쁘다. dex 의 엄지 해는 dex 의 손가락
  관절을 전제로 나온 값이라, 검지를 `ours` 것으로 갈아 끼우면 맞춰 놓은 거리가 깨진다.
  시간도 두 배(11.8ms).

**부작용 하나 — 손 크기에 민감해졌다.** 나머지 파이프라인은 배율을 스스로 재서
상쇄하는데, `project_dist`/`escape_dist` 는 절대 길이라 상쇄되지 않는다.
`p1_2_test_retarget_roundtrip.py` 가 이것을 잰다(가짜 손 크기를 바꿔 가며 결합이
엄지 목표를 얼마나 끌어당기는지):

```
손바닥폭 30mm -> 엄지 목표 50.3mm 이동, 손끝 잔차 4.31mm
         40mm ->              30.1mm,            1.34mm
         50mm ->               9.4mm,            1.41mm   <- 실측 사람 손 49.5mm
         60mm ->               4.5mm,            0.63mm
         70mm ->               0.9mm,            0.32mm
```

손이 작으면 벌린 자세도 "붙이려는 중"으로 읽혀 엄지가 검지 쪽으로 딸려 간다.
두 상수는 upstream(DexPilot) 값 그대로 두고 바꾸지 않는다 — upstream 도 실제
MediaPipe 랜드마크(진짜 미터)에 같은 절대값을 쓴다. 손이 작은 사용자에게서
엄지가 과하게 딸려 오면 그때 다시 볼 문제다.

같은 이유로 왕복 시험(`p1_2_test_retarget_roundtrip.py`)의 본 판정은 결합을 **끄고**
돈다. 그 시험은 "배율이 상쇄되는가"를 보는데 결합은 정의상 상쇄되지 않기 때문이다.

남은 결함: **주먹에서 54.2mm 에 멈춘다**(엄지 잔차 52mm). 목표는 0 인데 못 간다.
`ours` 의 주먹은 MCP 를 58° 접어서 검지 끝이 손바닥 깊숙이 들어가는데, 거기까지
엄지가 못 따라간다. dex 가 이 자세에서 31.6mm 를 내는 건 MCP 를 안 접어서 검지 끝이
바깥에 있기 때문이다. 사람도 주먹에서는 엄지가 손가락 **바깥**을 감싸지 손끝에 닿지
않으므로(실측 29.5mm), 애초에 목표를 0 으로 미는 것이 맞는지부터 다시 볼 문제다.
