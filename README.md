...
| **실기** `if_mcp` 1관절 (kP 600, 4 Mbps) | **99 ms** | **101 ms** | **1.89°** |
...
실기 첫 측정(2026-08-21, 데드맨 → `if_mcp` 한 관절 20°): 지연 99 ms / 상승 101 ms / 정상오차 1.89°,
표본 43. 데드맨 ON → 실기 자세 동기 → 램프 → 정지 순서가 설계대로 동작했다. 실기 읽기는
4 Mbps 에서 약 3% 가 CRC 로 깨져 직전 값으로 대체된다(`scripts/phase0/p0_4_read_reliability.py`,
방식·주기와 무관 → 배선/전기 쪽). 다음: 검지 3관절 → 16관절 전체 → 웹캠 + 실기.
