# 디버깅 사례집: 증상 → 가설 → 측정 → 수정 → 재측정

> 원문: `../history/phase1_retrospective.md` 3장(15건), `../design/ros2_twin.md`, 커밋 로그

개발 중 실제로 겪은 문제를 증상 → 원인 → 수정 순으로 정리했다. 같은 증상을 만나면 여기부터 본다.
사례마다 재현 방법이 있다. 🤖 는 실기에서만 재현된다.

## 이 저장소가 쓴 디버깅 규칙

1. **증상을 숫자로 바꾼다** (`measurement.md` 의 지표). "떤다"가 아니라 "명령 0.75°, 실기 4.8°(th_cmc)".
