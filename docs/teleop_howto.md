...
배율(`scaling_factor`)은 손 크기에 달렸다. 기본 1.0 은 이 저장소에서 실측한 값이고,
손이 크게 다르면 `python scripts/tune_dex_scale.py` 로 12초 찍어 다시 고른다.
...
| 로봇이 덜 움직임 (dex) | `--dex-scale 1.2` 처럼 올린다 (기본 1.0) |
