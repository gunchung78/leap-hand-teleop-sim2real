...
시작하면 **엄지 정렬 캘리브레이션이 돈다. 자세 두 개를 시킨다.**

1. **편 손** — 엄지가 손바닥 대비 놓인 기준 방향
2. **엄지를 손바닥에 붙인 손** (네 손가락은 편 채로) — 엄지가 접히는 방향

각각 3초 카운트다운 뒤 30프레임을 모은다. 끝나면 `회전축(roll) 결정됨: 예` 가
나와야 한다. 여기서 `아니오` 가 나오면 접기 자세가 안 잡힌 것이고, **엄지가
움직이긴 해도 손바닥으로 접히지 않는다.** 다시 실행할 것.

자세가 왜 둘인가: 하나로는 정렬 회전이 안 정해진다. 벡터 하나를 벡터 하나로 보내는
회전은 그 축 둘레의 roll 만큼 자유도가 남는다(3 중 2 만 고정). 그러면 엄지 목표가
LEAP 작업공간 안에 들어가긴 해서 잔차는 작게 나오는데, 방향이 틀려서 사람의
'손바닥으로 접기'가 `th_mcp`(굽힘) 대신 `th_cmc`(벌림) 로 새어 들어간다.

끝나면 본 루프가 시작된다. `q`(카메라 창 포커스) 또는 Ctrl-C 로 종료.
...
| 엄지가 손바닥으로 안 접힘 | 캘리브레이션에서 `roll 결정됨: 예` 였는지 확인. 아니면 재실행 |
