...
|---|---|---|
| 주먹 MCP/PIP/DIP (사람 44/89/92°) | **33/64/117** | 26/**3**/115 (PIP 를 안 쓴다) |
| 핀치 간격 (사람 27mm) | 54 mm | **38 mm** |
| 지터 (정지 자세) | 0.8~4.7° | 0.1~1.1° |
| 캘리브레이션 | 필요 | 없음 |

dex 는 손목->손끝 **벡터**만 목표로 잡아서, 손끝 위치만 맞으면 어떻게 접든 벌점이
없다. 그래서 주먹을 쥐면 PIP 를 안 쓰고 굽힘을 DIP 한 관절에 몰아넣는다. 숫자로는 잘
맞지만 눈으로는 **주먹이 아니라 갈고리**다. 이것이 "손 관절을 인식해도 로봇이
손처럼 안 움직인다"의 정체였다. ours 는 앞마디까지 목표로 주므로 세 관절에 나뉜다.
대신 사람과 LEAP 의 비율 차이 때문에 엄지-검지가 끝까지 안 닿는다(README 엄지 절).
...
| 재시도 | IK 가 다른 시드로 다시 푼 횟수 | 대부분 0 (평소 잔차 대비 2.5배 나쁠 때만 돈다) |
...
| 편 손인데 엄지가 꺾여 있음 / 엄지가 손바닥에 안 붙음 (ours) | `--thumb-mode map`(기본)인지 확인. `ik` 는 예전 동작이다 |
