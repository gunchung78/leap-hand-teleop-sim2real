...
def generate_launch_description():
    args = [
        ...
        DeclareLaunchArgument("smoothing", default_value="0.2", description="리타겟 지수 평활. 라이브 확인 0.2 (0.4 는 떨림)"),
        DeclareLaunchArgument("max_speed", default_value="8.0"),
        DeclareLaunchArgument("deadband", default_value="1.0",
                              description="리타겟 출력 데드밴드(deg). 정지 떨림 제거. 0=끔"),
        DeclareLaunchArgument("restart_mm", default_value="50.0",
                              description="IK 재시도 임계(mm). 50 이면 재시도 0 (엄지 잔차 30mm+). 1.0 은 매 프레임 5회 재시도 = 떨림"),
        DeclareLaunchArgument("pip_target", default_value="true",
                              description="PIP 관절점을 IK 목표에 추가 (손가락당 3점). 정지 떨림 감소"),
        DeclareLaunchArgument("tip_mode", default_value="axis",
